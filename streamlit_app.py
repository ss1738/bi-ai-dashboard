...
with TABS[0]:
    ...
    # Quick jump & copy link actions for Download section
    col_jump, col_copy = st.columns([1,1])
    with col_jump:
        if st.button("⬇️ Jump to Download Data", key="jump_dl"):
            st.components.v1.html("""
            <script>
            window.location.hash = '#download-data';
            setTimeout(() => {
            ...
            """, height=0)
    with col_copy:
        st.button("🔗 Copy download link", key="copy_dl")
        st.components.v1.html("""
        <script>
        const btn = window.parent.document.querySelector('button[kind="secondary"]:contains("Copy download link")');
        </script>
        """, height=0)
        st.components.v1.html("""
        <script>
        (function(){
          ...
          // Create a small invisible trigger div that Streamlit renders right after the button
          const triggers = document.getElementsByTagName('script');
          const me = triggers[triggers.length-1];
          if(me && !window.__dlCopyBound){
            window.__dlCopyBound = true;
            setTimeout(()=>{
              const btns = Array.from(window.parent.document.querySelectorAll('button'));
              const copyBtn = btns.find(b=>/Copy download link/.test(b.textContent));
    ...
    # ===== Scenario Simulator with Save & Compare =====
...
with TABS[4]:
    ...
    if df.empty:
        ...
        ccols = st.columns(min(3, len(cards))) if cards else []
        for i, (title, text) in enumerate(cards):
            with ccols[i % len(ccols)]:
                with st.container(border=True):
                st.markdown(f"🔹 {metric}: {value} — {text}")
            ...
            try:
                ...
                if any(k in ql for k in ["highest","top","max"]) and rev_col:
                    ...
                    for c in [col for col in df.columns if df[col].dtype == object]:
                        if c.lower() in ql: target_cat = c; break
